microservice.service("confirmOrderService", function($http, $location, loginService) {
	...
	this.saveorder = function(productid, username, paymentMode, billingAddress,
		...
		var responsePromise = $http({
			url : "https://s-apps.bsc.aws.societegenerale.com/isadco/tempAppApiGateway/api/order/isadco/tempAppOrderService/orders/createOrder",		
			method : "POST",
			data : orderdetails,
			headers : {
	...
	this.previousorders = function(username,jwttoken, callback) {
		...
		var responsePromise = $http({
			url : "https://s-apps.bsc.aws.societegenerale.com/isadco/tempAppApiGateway/api/order/isadco/tempAppOrderService/orders/previousOrders",			
			method : "POST",
			data : previousorders,
			headers : {
				'Content-Type' : 'application/json',
